const { consoleLoggerLevel } = require("vtex/lib/api/logger");
      ...
      $(".prateleira.vitrine ul li.helperComplement").length > 0 && $(".helperComplement").remove(), null == vitrine_html_element && (vitrine_html_element = ".vitrine"), null == html_child_element && (html_child_element = ".prateleira.vitrine > ul > li").not('.swatches-loaded'), $(vitrine_html_element).find(html_child_element).each(function () {
        if ($(this).find(".similar-colors-vitrine").length > 0) {
          try {
            ...
            if (product_link) {
              var apiurlSimilares = '/api/catalog_system/pub/products/crossselling/similars/' + product_id;
              $.ajax({
                url: apiurlSimilares,
                ...
                success: function (data, textStatus, jqXHR) {
                  ...
                  if(data) {
                    for (s=0; s < data.length; s++){
                      var product_similar = data[s].items[0].itemId;
                      //console.log("este el sim", product_similar)
                      apiurl = '/api/catalog_system/pub/products/search/?fq=skuId:' + product_similar;
                      $.ajax({
                        url: apiurl,
                        error: function (jqXHR, textStatus, errorThrown) {          
                        },
                            ...
                            if (data[0].items != null && data[0].items != void(0) && data[0].items.length > 0) {                              
                              ...
                              for (var i = 0; i < skus.length; i++) {
                                ...
                                if (skus[i].Color !== null && skus[i].Color !== undefined && skus[i].Color.length > 0 && skus[i].Color[0].length > 0 ) {
                                  ...
                                  if (0 === i || skus[i].Color[0] !== skus[before].Color[0] && $container_variation.find('li[data-sku-color="' + skus[i].Color[0] + '"]').length < 1 && count < 6) {
                                    ...
                                    /*const elementos = document.querySelectorAll("#skuID-oculto");
                                    var elemento = null;
                                    for(e=0; e < elementos.length;e++){
                                      if(elementos[e].innerHTML == product_sku){

                                       }
                                      elementos[e]
                                    }*/
                                    var elemento = $product_element_object.find("#skuID-oculto").parent().find(".product-variation")[0];
                                    //console.log("El elemento", elemento);
                                    var cuenta = jQuery($product_element_object).find(".product-variation li").length;
                                    if (cuenta < 5) {
                                    jQuery($product_element_object).find(".product-variation").append('<li id="variacao-item-' + sku + '" data-sku-color="' + color + '" data-sku-thumb=' + prodvariation_img.replace(skus[i].images[0].imageId, skus[i].images[0].imageId + "-238-238") + ' data-sku-thumb_atras=' + prodvariation_img_atras.replace(skus[i].images[0].imageId, skus[i].images[0].imageId + "-238-238")+'> <a href="'+ linkVariation +'"class="wrap-image" style="background-color:' + color + '"></a></li>'),
